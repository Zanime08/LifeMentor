...
#
# What it does:
#   1) builds the production server bundle (apps/server/dist/main.mjs)
#   2) writes a hidden launcher (server-run.vbs) that logs to server.log
#   3) registers a scheduled task "LifeMentor Server" (runs at logon, no window)
#
...
Write-Host '1/3 Building the production server bundle...'
npm run build:server
if ($LASTEXITCODE -ne 0) { throw 'npm run build:server failed' }

Write-Host '2/3 Writing the hidden launcher...'
$vbs = @"
...
Write-Host '3/3 Registering the scheduled task "LifeMentor Server" (at logon)...'
