...
**Test suite today:** 234 tests, 38 files — `packages/core/test` (public API, persistence + WASM
...
3. **Polish & hardening** (in progress, 2026-09-09):
   ...
   - **DONE — hardening round (2026-09-10)**, found by the new UI/planner tests:
     ...
     * **a failed read looked exactly like an empty account**: every screen ended its loaders with
       ...
       showed an empty list indistinguishable from real, empty data. Screens now read through
       `loadSafely`, which records the failure: the user gets the short Russian sentence, a
       «Повторить» button and the note that nothing on disk was touched, while the internal message
       stays in the console for support. Two places were *lying*, not merely hiding: the project form
       showed an empty goal list (now «не удалось прочитать цели»), and the event form's overlap
     ...
     * **a tool that had not understood the request reported success**: when the model named a task
       («отмени лабораторную») and the reference fitted more than one open task, the tool returned its
       clarification as a *successful* result — a ✓ «отменил задачу» chip on a call that cancelled
       nothing — and the sentence it printed was the one written *for the model*: «Ask the user which
       one they mean — do not pick one yourself». The refusal is now a refusal (`ok: false`) and
       carries the facts as data (`needs_input_item`: ambiguous / not_found, the words the user used,
       the candidates), which the engine words for the reader (`needsInputItemText` — «Под
       «лабораторную» подходит несколько: «Сдать лабораторную по физике», «Сдать лабораторную по
       химии». Какой именно вы имели в виду?») and the chat turns into a chip that names the reason
       («⚠ под «лабораторную» подходит несколько — уточните») instead of the past tense of a success.
       The model's instruction stays in the tool result, where it belongs.
...
npm test                    # 234 tests (core + server + shell driver contracts + web bootstrap + UI journeys)
