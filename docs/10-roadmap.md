...
| 18 | Testing (persistence, sync, AI, planner, learning, notifications, API, UI) | ✅ 154 automated tests passing (23 files: core + server + WASM driver durability + **Tauri/Capacitor driver contract tests** + browser bootstrap + push engine (incl. **FCM v1 unit + integration**) + **LLM news enrichment** + cloud backup + **planner phase gate** + **bundle boot test (builds `dist/main.mjs` and calls the running server)** + **`init:env` test** + **maintenance phase gate (snapshots/reviews/backup/retention on a real clock, 10 tests)** + **UI journey tests driving the real client under jsdom**) |
| 19 | Packaging (Windows NSIS/MSI, Android APK, server release bundle) | 🟡 **one command away** — server bundles to `dist/main.mjs` and *is verified to boot and serve* by `apps/server/test/bundle.test.ts` (the bundle once died with `Dynamic require of "crypto"`); `npm run init:env` writes a stable `.env` (JWT secret + VAPID pair) so sessions and push survive restarts; shell projects + CI release workflow (`release.yml`: tests → NSIS .exe on windows-latest, APK on ubuntu-latest) are in place; the binaries are produced on a machine with Rust/JDK (or by tagging `v*`) — `docs/11-packaging.md` |
...
**Test suite today:** 154 tests, 23 files — `packages/core/test` (public API, persistence + WASM
...
npm test                    # 154 tests (core + server + shell driver contracts + web bootstrap + UI journeys)
